...
const HistorialScreen = ({ navigation }) => {
  ...
  const cargarRutas = useCallback(async () => {
    try {
      ...
      // ✅ Obtener TODAS las rutas (finalizadas y activas)
      const rutasUsuario = await databaseService.obtenerRutasUsuario(usuario.id);
      
      // ✅ Obtener conteo de productos para cada ruta
      const rutasConConteo = await Promise.all(
        rutasUsuario.map(async (ruta) => {
  ...
  const handleCompartir = async (ruta) => {
    try {
      ...
      if (!usuario) {
        Alert.alert('Error', 'Usuario no disponible para el reporte.');
        return;
      }

      const reporteTexto = formatters.generarReporteTexto(ruta, usuario, productos, 45000000000, 47000000000);
      
      // ✅ CORREGIDO: Asegurar saltos de línea reales para WhatsApp
      const mensajeFormateado = reporteTexto.replace(/\\n/g, '\n').replace(/\n{3,}/g, '\n\n');
      
      console.log('📤 Reporte generado:', mensajeFormateado);
      
      Alert.alert(
        'Compartir Reporte',
        '¿Deseas compartir este reporte por WhatsApp?',
        [
            ...
            onPress: async () => {
              ...
              if (Share) {
                await Share.share({
                  ...
                  title: `📋 Reporte Ruta ${ruta.numero}`
                });
              }
            }
          }
  ...
  // ✅ NUEVA FUNCIÓN: Limpiar mellizos de una ruta
  const handleLimpiarMellizos = async (ruta) => {
    Alert.alert(
      '🔄 Limpiar Mellizos',
      `¿Desea eliminar la marca de "mellizo" de todos los productos de la ruta ${ruta.numero}?`,
      [
        { text: 'Cancelar', style: 'cancel' },
        {
          text: 'Limpiar',
          ...
          onPress: async () => {
            try {
              ...
              if (mellizos.length === 0) {
                Alert.alert('ℹ️ Info', 'No hay mellizos para limpiar en esta ruta');
                return;
              }
              ...
              console.error('❌ Error limpiando mellizos:', error);
              Alert.alert('Error', 'No se pudieron limpiar los mellizos');
            }
          }
        }
      ]
  ...
  const handleEliminar = useCallback((ruta) => {
    Alert.alert(
      ...
      `¿Eliminar la ruta ${ruta.numero} y todos sus productos?`,
      [
        { text: 'Cancelar', style: 'cancel' },
        {
          text: 'Eliminar',
  ...
  const renderItem = useCallback(({ item }) => (
    <Card style={styles.rutaCard}>
      <Card.Content>
        ...
        <View style={styles.rutaStats}>
          <Text style={styles.statsText}>
            <Text style={styles.statsCount}>{item.total_productos}</Text> paquetes
          </Text>
          <Text style={[styles.estadoBadge, item.finalizada ? styles.finalizada : styles.activa]}>
      ...
      <Card.Actions style={styles.cardActions}>
        <Button
          mode="outlined"
          onPress={() => handleVerProductos(item)}
        ...
        {/* ✅ NUEVO BOTÓN: Limpiar Mellizos */}
        <Button
          mode="outlined"
          textColor="#ff9800"
          onPress={() => handleLimpiarMellizos(item)}
          style={styles.actionButton}
          icon="refresh"
        >
          Mellizos
