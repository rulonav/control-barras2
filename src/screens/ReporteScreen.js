...
const ReporteScreen = ({ route, navigation }) => {
  const { ruta, productos: productosIniciales, limitesEscaneo } = route.params || {};
  ...
  useEffect(() => {
    // Aplicar límites si vienen en params
    if (limitesEscaneo) {
    ...
    try {
      ...
      if (isNaN(fechaRuta.getTime())) {
        setEsDelDia(false);
        return;
      }

      // Comparar usando hora local del dispositivo
      const ahora = new Date();
      const hoy = new Date(ahora.getFullYear(), ahora.getMonth(), ahora.getDate());
      const fechaRutaLocal = new Date(fechaRuta.getFullYear(), fechaRuta.getMonth(), fechaRuta.getDate());
      const esHoy =
        hoy.getDate() === fechaRutaLocal.getDate() &&
        hoy.getMonth() === fechaRutaLocal.getMonth() &&
        hoy.getFullYear() === fechaRutaLocal.getFullYear();
  ...
  }, [ruta?.fecha, limitesEscaneo]);

  const limpiarMellizo = async (id) => {
    try {
      await databaseService.actualizarProducto(id, { es_mellizo: 0 });
      setProductos(prev => prev.map(p => p.id === id ? { ...p, es_mellizo: 0 } : p));
    } catch (error) {
      console.error('❌ Error al limpiar mellizo:', error);
      Alert.alert('Error', 'No se pudo limpiar el producto.');
    }
  };
  ...
  const limpiarTodosMellizos = async () => {
    try {
      ...
      for (const p of mellizos) {
        await databaseService.actualizarProducto(p.id, { es_mellizo: 0 });
      }
      setProductos(prev => prev.map(p => ({ ...p, es_mellizo: 0 })));
      Alert.alert('Éxito', `${mellizos.length} productos limpiados.`);
    } catch (error) {
      console.error('❌ Error al limpiar todos los mellizos:', error);
      Alert.alert('Error', 'No se pudieron limpiar los productos.');
    }
  };
  ...
  const agregarPaquetes = () => {
    navigation.navigate('Scanner', {
      rutaRecuperada: ruta,
      limitesEscaneo: {
        inferior: limiteInferior / 1000000000,
        superior: limiteSuperior / 1000000000
      }
    });
  };
  ...
  // ✅ CORREGIDO: Compartir reporte con saltos de línea reales para WhatsApp
  const compartirReporte = async () => {
    try {
      const usuario = await databaseService.obtenerUsuarioActual();
      
      // Generar reporte con formatters (saltos de línea corregidos)
      const reporteTexto = formatters.generarReporteTexto(
        ruta,
        usuario,
        productos,
        limiteInferior,
      ...
      // ✅ CORREGIDO: Asegurar que los saltos sean interpretados por WhatsApp
      const mensajeFormateado = reporteTexto.replace(/\\n/g, '\n').replace(/\n{3,}/g, '\n\n');

      await Share.share({
        message: mensajeFormateado,
        title: `📋 Reporte Ruta ${String(ruta?.numero ?? 'Desconocida')}`
      });
    } catch (error) {
  ...
  const renderItem = ({ item, index }) => {
    ...
    // Filtrar por rango
    if (codigoNum < limiteInferior || codigoNum > limiteSuperior) {
      return null;
    }
      ...
      <View style={styles.row}>
        ...
        {esMellizo && esDelDia && (
          <Button
            mode="text"
            textColor="#f44336"
            onPress={() => limpiarMellizo(item.id)}
          >
            Limpiar
          </Button>
        )}
  ...
  const tituloRuta = `Ruta: ${String(ruta?.numero ?? 'Desconocida')}`;
  const fechaTexto = String(formatters.formatFecha(ruta?.fecha, true) || 'Fecha no disponible');

  // Filtrar productos por rango para mostrar
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <Text style={styles.emptyText}>No hay productos en el rango seleccionado</Text>
        }
      />
      ...
      {esDelDia && (
        <>
          <FAB
            ...
            onPress={agregarPaquetes}
            style={[styles.fab, { bottom: 180 }]}
            color="#fff"
          />
          <FAB
            icon="alert"
            label="Agregar Dañados"
            onPress={agregarDanados}
